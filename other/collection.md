...
- 用户角度：一次登录多次使用，无需记录多套用户名和密码。

- 开发者角度：只需维护好一个统一的账号中心就可以了。
...
将登陆功能抽取为一个系统 SSO，其他系统请求 SSO 进行登录本来将用户信息存到 Session，现在将用户信息存到 Redis。
...
Central Authentication Service，SSO 认证中心类似一个中转站。
...
你是谁。验证您的身份的凭据，例如用户名和密码。

**授权 Authorization**

你有权限干什么。发生在认证之后，主要掌管我们访问系统的权限。
...
授权机制，它的最终目的是为第三方应用颁发一个有时效性的令牌 Token，使得第三方应用能够通过该令牌获取相关的资源。
