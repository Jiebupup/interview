...
负载均衡在实际项目操作过程中，有硬件负载均衡和软件负载均衡两种。硬件负载均衡造价昂贵成本较高，但是数据的稳定性安全性等等有非常好的保障。

#### **负载均衡器**

负载均衡器会根据集群中每个节点的负载情况，将用户请求转发到合适的节点上。

单体式应用通过在负载均衡器后端运行多个拷贝，就可以轻松实现应用扩展。
...
在轮询的基础上，根据服务器的性能差异，为服务器赋予一定的权值，性能高的服务器分配更高的权值。权重越大，被分配到请求的几率越大。

默认的负载均衡算法。

即使在使用过程中，某一台后端服务器宕机，Nginx 会自动将该服务器剔除出队列，请求受理情况不会受到任何影响。
