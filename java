...
成员变量如果没有被赋初值:如果是基本数据类型则会自动以类型的默认值而赋值（一种情况例外被 final 修饰的成员变量也必须显示地赋值），如果是对象的引用则必须初始化；而局部变量则不会自动赋值。
...
接口也可以拥有默认的方法实现（java8开始），接口的成员（字段 + 方法）默认都是 public ，接口的字段默认都是 static 和 final 的。
在很多情况下，接口优先于抽象类，比较：
